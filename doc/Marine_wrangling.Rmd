...
Assume same slope but different intercepts in every subgroup (which is every habitat and feeding type).

```{r}
lm3 <- lmer(log(Prey_mass) ~ log(Predator_mass) + (1|Specific_habitat) + (1|Type_of_feeding_interaction), data = filtered_dataset)
```
...
```{r}
lm4 <- lmer(log(Prey_mass) ~ log(Predator_mass) + (1 + log(Predator_mass)
                                            |Type_of_feeding_interaction), data = filtered_dataset)
summary(lm4)
ranef(lm4)
tidy(lm4,conf.int = T)
```
...
Assuming different slopes for feeding type but with same intercept. 
```{r}
lm5 <- lmer(log(Prey_mass) ~ log(Predator_mass) + (log(Predator_mass) - 1|Type_of_feeding_interaction), data = filtered_dataset)
...
Considers slope itself. 

```{r}
lm6 <- lmer(log(Prey_mass) ~ log(Predator_mass) + (log(Predator_mass) - 1|Type_of_feeding_interaction) + (1|Specific_habitat), data = filtered_dataset)
...
```{r}
lm7 <- lmer(log(Prey_mass) ~ log(Predator_mass) + Type_of_feeding_interaction * Specific_habitat * log(Predator_mass) + (1|Specific_habitat) + (1|Type_of_feeding_interaction), data = filtered_dataset)
...
```{r}
model.sel(lm1, lm2, lm3, lm4, lm6, lm7, rank = AIC)
```
